        ...
        <div className="border-b border-slate-700 mb-6">
            <nav className="flex -mb-px gap-4">
                <button
                    ...
                    className={`py-3 px-2 text-sm font-medium border-b-2 transition-colors duration-200 ${
                        activeTab === 'catalogs'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/50 p-4 rounded-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="item-name" className="block text-sm font-medium text-slate-400 mb-1">Name</label>
                        <input
                            id="item-name"
                            type="text"
                            value={name}
                    ...
                    <div>
                        <label htmlFor="item-url" className="block text-sm font-medium text-slate-400 mb-1">URL</label>
                        <input
                            id="item-url"
                            type="url"
                            value={url}
            ...
            <div className="bg-slate-900/50 p-4 rounded-lg">
                ...
                {currentList.length > 0 ? (
                            ...
                            <li key={item.id} className="bg-slate-700/50 p-2 rounded-md flex items-center justify-between gap-2">
                                {editingItemId === item.id ? (
                                    ...
                                    <>
                                        ...
                                        <div className="flex items-center flex-shrink-0">
                                            <button onClick={() => handleStartEdit(item)} className="p-2 rounded-full hover:bg-sky-500/20 text-slate-500 hover:text-sky-400 transition-colors" aria-label={`Edit ${item.name}`}>
                                                <PencilIcon className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(item.id)} className="p-2 rounded-full hover:bg-red-500/20 text-slate-500 hover:text-red-400 transition-colors" aria-label={`Delete ${item.name}`}>
                    ...
                    <p className="text-sm text-slate-400 text-center py-8">No {activeTab === 'catalogs' ? 'catalogs' : 'registries'} added yet.</p>
                )}
            </div>
        </div>
      </div>
